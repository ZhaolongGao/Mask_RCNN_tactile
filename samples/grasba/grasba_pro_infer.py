...
# GrasbaPro 模型
from samples.grasba import grasba_pro
...
# Path to GrasbaPro trained weights
GRASBAPRO_WEIGHTS_PATH = os.path.join(ROOT_DIR, "mask_rcnn_grasba_pro_0026.h5")

# GrasbaPro 设置文件
config = grasba_pro.GrasbaProConfig()
GRASBAPRO_DIR = os.path.join(ROOT_DIR, "datasets/grasba_pro")
...
def apply_effect(image, result, colors):
    ...
    class_names = ['BG', 'hand', 'hollow_ball', 'wooden_ball', 'line_ball', 'spiked_ball']
    scores = p['scores']
...
# 加载预训练模型
# Set path to grasba_pro weights file
weights_path = GRASBAPRO_WEIGHTS_PATH
...
# 加载数据
# 图像和视频选择
image_path = os.path.join(GRASBAPRO_DIR, "val", "red_ball.png")
image_path = None
video_path = os.path.join(GRASBAPRO_DIR, "val", "test.avi")
# video_path = None
# 标注颜色选取
MAX_CLASSES = 5
